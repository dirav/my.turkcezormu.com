...
use MasterStudy\Lms\Pro\AddonsPlus\AiLab\Http\Serializers\CourseSerializer;
...
class GenerateCourseController extends Controller {
	public function __invoke( \WP_REST_Request $request ) {
		...
		try {
			$response = $this->ai->generate_course( $validated_data['prompt'] );

			return new WP_REST_Response(
				( new CourseSerializer() )->toArray( $response )
			);
		} catch ( \Exception $e ) {
