...
document.addEventListener('DOMContentLoaded', function() {
    setTimeout(() => {
        if (jQuery.fn.select2) {
            const selectElement = jQuery('select.masterstudy-grades-instructor__select');
    ...
    document.addEventListener('click', (event) => {
        const activeDropdowns = document.querySelectorAll('.masterstudy-grades-instructor__search-dropdown_active');
        ...
        if (activeDropdowns.length) {
            activeDropdowns.forEach((dropdown) => {
                const parent = dropdown.closest('.masterstudy-grades-instructor__search-wrapper');
                const input = parent.querySelector('input');
                const isClickInsideInput = input && input.contains(event.target);

                if (!isClickInsideInput) {
                    dropdown.classList.remove('masterstudy-grades-instructor__search-dropdown_active');
                }
            });
        }
        ...
        if (row) {
            ...
            const fetchPath = `grades/${userCourseId}`;
    ...
    dateSelect.addEventListener('change', function(event) {
        ...
        selectedPeriod = defaultDateRanges[selectedValue];
    ...
    if (document.querySelectorAll('input[id*="-filter"]').length) {
        document.querySelectorAll('input[id*="-filter"]').forEach(function (selector) {
            selector.addEventListener('input', function () {
                ...
                const dropdown = this.parentNode.querySelector(`.masterstudy-grades-instructor__search-dropdown`);
                let fetchPath = '';

                if (inputId === 'courses-filter') {
                    fetchPath = `search?search=${encodeURIComponent(query)}&type=stm-courses`;
                } else if (inputId === 'students-filter') {
                    fetchPath = `users?search=${encodeURIComponent(query)}`;
                }

                if (!dropdown || !fetchPath) return;
                    ...
                    wpApi.get(fetchPath).then(result => {
                        ...
                        if (result.length > 0) {
                            result.forEach(item => {
                                ...
                                dropdownItem.className = `masterstudy-grades-instructor__search-dropdown-item`;
                                dropdownItem.setAttribute('data-id', item.id);
                                dropdownItem.textContent = item.name ?? item.title;
                                dropdown.appendChild(dropdownItem);
                            });
                            dropdown.classList.add(`masterstudy-grades-instructor__search-dropdown_active`);
                        } else {
                            dropdown.classList.remove(`masterstudy-grades-instructor__search-dropdown_active`);
                        }
                    })
    ...
    document.querySelector('[data-id="regenerate-grades"]').addEventListener('click', function(event) {
        ...
        const gradeLoader = document.querySelector('.masterstudy-grade-details__loader');
        const fetchPath = `grades/${userCourseId}/regenerate`;
                ...
                return `
                    <div class="masterstudy-grade-details__exams-item masterstudy-grade-details__exams-item-${lesson.type}">
                        ...
                        ${lesson.grade && Object.keys(lesson.grade).length > 0 ? `
                            <div class="masterstudy-grade-details__exams-item-attempt">${lesson.attempts} ${grades_instructor_data.attempts}</div>
                            <div class="masterstudy-grade-details__exams-item-grade">
                                ...
                                <div class="masterstudy-grade-details__exams-item-grade-value">(${lesson.grade.current}${grades_instructor_data.grade_separator}${lesson.grade.max_point})</div>
                            </div>
                            <div class="masterstudy-grade-details__exams-item-percent">${lesson.grade.range}%</div>
                            `
                            :
                            `
                            <div class="masterstudy-grade-details__exams-item-start">${grades_instructor_data.not_started}</div>
                            `
                        }
                    </div>
                `;
...
function updateTable(reloadTable = false) {
    ...
    let columnDefs = [
        ...
        },
        {
            ...
            data: 'quiz',
            render: function (data, type, row) {
                return data.complete + grades_instructor_data.grade_separator + data.total;
            }
        },
        {
            targets: 4,
    ...
    table = updateDataTable(
        ...
        'grades',
        grades_instructor_data.columns,
        dataSrc,
        columnDefs,
        reloadTable,
        false,
