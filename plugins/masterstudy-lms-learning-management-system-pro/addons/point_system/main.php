...
class STM_LMS_Point_System {
	...
	public function __construct() {
		...
		add_action( 'stm_lms_lesson_passed', array( $this, 'lesson_passed' ), 10, 3 );
		add_action( 'stm_lms_quiz_passed', array( $this, 'quiz_passed' ), 10, 4 );
		add_action( 'stm_lms_assignment_passed', array( $this, 'assignment_passed' ), 10, 3 );
	...
	public function lesson_passed( $user_id, $lesson_id, $course_id ) {
		...
		if ( method_exists( 'STM_LMS_Course', 'is_course_complete' ) ) {
			if ( STM_LMS_Course::is_course_complete( $user_id, $course_id ) ) {
	...
	public function assignment_passed( $user_id, $assignment_id, $course_id ) {
